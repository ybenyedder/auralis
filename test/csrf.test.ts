...
test("logout, lyrics refetch and lyrics align routes reject a forged cross-origin POST (403, before auth even runs)", async () => {
  // Regression lock for a real gap: these three routes never called checkCsrf(),
  // unlike every other cookie-authed mutation. Each should now reject on Origin
  // alone — 403, not 401 — proving the CSRF guard runs before getRequestUser().
  const { POST: logoutPost } = await import("../src/app/api/auth/logout/route");
  const { POST: lyricsPost } = await import("../src/app/api/lyrics/[trackhash]/route");
  const { POST: alignPost } = await import("../src/app/api/lyrics/[trackhash]/align/route");

  const hostile = { origin: "http://evil.test", host: "localhost:4237" };
  const ctx = { params: Promise.resolve({ trackhash: "deadbeef" }) };

  const logoutRes = await logoutPost(req("POST", hostile));
  assert.equal(logoutRes.status, 403);

  const lyricsRes = await lyricsPost(new Request("http://localhost:4237/api/lyrics/deadbeef", { method: "POST", headers: hostile }), ctx);
  assert.equal(lyricsRes.status, 403);

  const alignRes = await alignPost(new Request("http://localhost:4237/api/lyrics/deadbeef/align", { method: "POST", headers: hostile }), ctx);
  assert.equal(alignRes.status, 403);
});
