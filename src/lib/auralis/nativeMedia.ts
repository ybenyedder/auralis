...
/** Drop every registered action handler — call when there's no current track so a
 *  lock-screen "play" can't fire a handler that flips isPlaying with nothing loaded. */
export function clearMediaHandlers() {
  if (!hasWebSession()) return;
  const ms = navigator.mediaSession;
  for (const action of ["play", "pause", "previoustrack", "nexttrack", "seekto", "seekbackward", "seekforward", "stop"] as const) {
    try {
      ms.setActionHandler(action, null);
    } catch {
      /* action type unsupported on this platform — nothing to clear */
    }
  }
}
