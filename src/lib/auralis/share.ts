 ...
 */
export async function shareTrack(track: Track, notify: (m: string) => void, locale: Locale = "fr"): Promise<void> {
  const label = `${trackTitle(track)} — ${trackArtist(track)}`;
  const text = translate(locale, "toast.shareText", undefined, { label });
