...
export async function PUT(request: Request) {
  ...
  switch (body.action) {
    ...
    case "playlist.generateFromSeeds": {
      ...
      // This scores the whole library per call, so throttle it per-account (20 /
      // minute) — the only PUT action heavy enough to be a self-inflicted DoS.
      if (rateLimitWindow(`seeds:${uid}`, 20, 60_000)) {
        return json({ error: "Trop de générations. Réessayez dans un instant." }, { status: 429, headers: { "Retry-After": "60" } });
      }
