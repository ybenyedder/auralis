...
import { useTabSwipe } from "@/lib/auralis/useTabSwipe";
...
function AuralisShell() {
  ...
  // Direction of the last committed tab swipe ("next" = towards Rechercher,
  // "prev" = towards Accueil). Set by useTabSwipe on commit so the incoming
  // view slides in from the matching side; cleared after the animation window
  // so a later plain navigation falls back to the standard push animation.
  const [tabSwipeDir, setTabSwipeDir] = useState<"next" | "prev" | null>(null);
  useTabSwipe(mainRef, setTabSwipeDir);
  ...
  useEffect(() => {
    ...
    void useRecap.getState().fetchRecap().then(() => {
      ...
      usePlayer.getState().notify(translate(usePlayer.getState().locale, "toast.recapReady", undefined, { month: monthLabelFr(elapsed, usePlayer.getState().locale) }), {
        tone: "info",
        action: {
          label: translate(usePlayer.getState().locale, "toast.viewAction"),
  ...
  useEffect(() => {
    ...
    try {
      if (sessionStorage.getItem("auralis.pwNudge")) {
        ...
        usePlayer.getState().notify(translate(usePlayer.getState().locale, "toast.initialPassword"), {
          tone: "info",
          action: { label: translate(usePlayer.getState().locale, "nav.settings"), run: () => usePlayer.getState().navigate("settings") },
        });
      }
    ...
    if (isPlaying) {
      ...
      audio.play().catch((err: unknown) => {
        if (!(err instanceof DOMException) || err.name !== "AbortError") {
          ...
          notify(translate(usePlayer.getState().locale, "toast.audioUnavailable"), { tone: "error" });
        }
      });
    } else {
      audio.pause();
  ...
  useEffect(() => {
    ...
    const onEnded = () => {
      ...
      if (state.sleepTimer.active && state.sleepTimer.endOfTrack) {
        ...
        state.notify(translate(state.locale, "toast.sleepDone"));
        return;
      }
    ...
    const onError = () => {
      ...
      if (consecutiveErrors === 1) state.notify(translate(state.locale, "toast.trackUnreadable"), { tone: "error" });
      if (consecutiveErrors >= Math.max(3, state.shuffledQueue.length)) {
        usePlayer.setState({ isPlaying: false });
        state.notify(translate(state.locale, "toast.playbackImpossible"), { tone: "error" });
        return;
      }
  ...
  // Clear the swipe direction AFTER the slide-in animation window so a plain
  // navigation right after a swipe still gets the correct animation, and the
  // next ordinary navigation falls back to the standard push.
  useEffect(() => {
    if (!tabSwipeDir) return;
    const id = window.setTimeout(() => setTabSwipeDir(null), 350);
    return () => window.clearTimeout(id);
  }, [tabSwipeDir, view.view, view.id]);
  ...
  return (
    <>
      ...
      <div className="sr-only" aria-live="polite" aria-atomic="true">
        {currentTrack ? t("shell.nowPlaying", undefined, { title: trackTitle(currentTrack), artist: trackArtist(currentTrack) }) : ""}
      </div>
      ...
      <div className="flex min-h-0 flex-1 md:gap-2 md:px-2 md:pt-2 pb-2 md:pb-0">
        ...
        >
          ...
          <ErrorBoundary area={t("error.area")} resetKey={`${view.view}-${view.id ?? ""}`}>
            <div
              key={`${view.view}-${view.id ?? ""}`}
              className={cn(
                "relative h-full",
                tabSwipeDir === "next" ? "am-tab-next" : tabSwipeDir === "prev" ? "am-tab-prev" : "am-push-enter",
              )}
            >
              {viewEl}
            </div>
        ...
        <ErrorBoundary area={t("shell.errorPanel")} compact>
          <NowPlayingPanel />
        </ErrorBoundary>
      </div>
      ...
      {fullscreenPlayer && (
        <ErrorBoundary area={t("shell.errorFullscreen")} compact>
          <FullscreenPlayer />
        </ErrorBoundary>
      )}
