...
export async function runScan(): Promise<ScanProgress> {
  ...
  try {
    ...
    void import("./analysis")
      ...
      // Cheap, pure-TS reco enrichment: score the lyrics we already hold for the
      // "cognitive dissonance" feature. Always runs (no ffmpeg / model / network).
      .then(() => import("../reco/lyricsSentiment").then((m) => m.runLyricsSentiment()))
      .catch(() => {/* lyric sentiment is best-effort */})
      // Optional deep audio embeddings (timbre/stems) via the Python extractor —
      // heavy, so strictly opt-in (AURALIS_EMBEDDINGS=1) and it no-ops otherwise.
      .then(() => import("../reco/embeddingExtract").then((m) => m.runEmbeddingExtraction()))
      .catch(() => {/* embedding extraction is best-effort / opt-in */});
  } catch (error) {
