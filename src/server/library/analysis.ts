...
export async function runAnalysis(): Promise<void> {
  ...
  // result is cached, so a flag armed only after it let two concurrent callers
  // (double-clicked "analyser", or a POST landing as the post-scan pass starts)
  // both pass the guard and spawn parallel worker pools on the same rows. Setting
  // it here + a try/finally that always clears it keeps exactly one pass alive and
  // never wedges the flag `true` if getDb()/the SELECT throws.
  if (analyzing) return;
  analyzing = true;
  ...
  try {
    ...
    analyzing = false;
    // Only touch progress if we actually announced a start, so an early return
    // (nothing pending / no ffmpeg) doesn't emit a spurious analyzing:false frame.
    if (started) updateScanProgress({ analyzing: false, analyzed: done, analyzeTotal: total });
  }
}
