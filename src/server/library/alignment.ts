...
// Bound the status map: it holds one entry per track ever aligned on demand and
// nothing purged it. When over the cap, evict the oldest non-running entry (a
// running one is still awaited by a poller). Old finished statuses are safe to
// drop — getAlignStatus just returns null and the client treats that as "idle".
const ALIGN_STATUS_MAX = 500;
function setAlignStatus(hash: string, status: AlignStatus): void {
  alignStatus.set(hash, status);
  if (alignStatus.size <= ALIGN_STATUS_MAX) return;
  let oldestKey: string | null = null;
  let oldestAt = Infinity;
  for (const [key, s] of alignStatus) {
    if (s.state === "running") continue;
    if (s.at < oldestAt) { oldestAt = s.at; oldestKey = key; }
  }
  if (oldestKey) alignStatus.delete(oldestKey);
}
...
async function runAlignOne(trackhash: string): Promise<void> {
  setAlignStatus(trackhash, { state: "running", at: Date.now() });
  const script = scriptPath();
  if (!script) {
    setAlignStatus(trackhash, { state: "unavailable", message: "Aligneur absent de cette build.", at: Date.now() });
    return;
  }
  const { musicDir, dataDir } = getConfig();
  ...
  await new Promise<void>((resolve) => {
    ...
    try {
      ...
      setAlignStatus(trackhash, { state: "unavailable", message: "Python introuvable sur le serveur.", at: Date.now() });
      return resolve();
    }
    ...
    child.on("close", async (code) => {
      ...
      if (/manquants|No module named|ModuleNotFoundError/i.test(err)) {
        setAlignStatus(trackhash, { state: "unavailable", message: "Dépendances IA non installées sur le serveur (torch).", at: Date.now() });
        return resolve();
      }
      if (out.includes("✓ MOT")) {
        ...
        setAlignStatus(trackhash, { state: "ok", at: Date.now() });
        log.info("forced alignment (on-demand) succeeded", { trackhash });
      } else {
        setAlignStatus(trackhash, {
          state: "failed",
