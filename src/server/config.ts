...
export function getConfig(): AuralisConfig {
  ...
  // Unlike dataDir/artDir, musicDir isn't created for the user — a fresh empty
  ...
  // musicDir field).
  try {
    fs.accessSync(/*turbopackIgnore: true*/ config.musicDir, fs.constants.R_OK);
  } catch {
    log.warn("configured music directory is missing or unreadable — library will scan as empty", {
