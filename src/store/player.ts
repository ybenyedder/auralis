...
// Monotonic toast id. Date.now() collides when two toasts fire in the same
// millisecond (a streak celebration + a scrobble on the same synchronous tick),
// and the first one's dismiss timer would then close the second, still-fresh
// toast early because they share an id.
let toastSeq = 0;
