...
createPlaylist: (name, description) => {
      ...
      get().notify(translate(get().locale, "toast.playlistCreated", undefined, { name: pl.name }));
      return id;
    },
...
createSmartPlaylist: (config) => {
      ...
      get().notify(translate(get().locale, "toast.smartCreated", undefined, { name: pl.name }));
      return id;
    },
...
deletePlaylist: (id) => {
      ...
      get().notify(translate(get().locale, "toast.playlistDeleted"));
    },
...
setPlaylistCover: async (id, dataUrl) => {
      try {
        ...
        get().notify(translate(get().locale, "toast.coverUploadFailed"));
      }
    },
...
addToPlaylist: (id, track) => {
      ...
      get().notify(translate(get().locale, "toast.addedToPlaylist", undefined, { name: pl?.name ?? translate(get().locale, "toast.thePlaylist") }));
    },
...
importPlaylist: (name, trackhashes) => {
      ...
      get().notify(translate(get().locale, "toast.playlistImported", undefined, { name: pl.name, count: unique.length }));
      return id;
    },
...
sharePlaylist: (id, shared) => {
      ...
      get().notify(translate(get().locale, shared ? "toast.shareEnabled" : "toast.shareDisabled"));
    },
...
addPlaylistCollaborator: async (id, username) => {
      try {
        ...
        get().notify(translate(get().locale, "toast.collaboratorAdded", undefined, { username }));
        return true;
      } catch {
        get().notify(translate(get().locale, "toast.collaboratorNotFound"), { tone: "error" });
        return false;
      }
    },
