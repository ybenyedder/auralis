...
startSleepTimer: (minutes) => {
      ...
      get().notify(translate(get().locale, "toast.sleepTimerSet", undefined, { minutes }));
    },

sleepAfterTrack: () => {
      set({ sleepTimer: { active: true, endsAt: null, minutes: 0, endOfTrack: true } });
      get().notify(translate(get().locale, "toast.sleepAtEnd"));
    },

cancelSleepTimer: () => {
      set({ sleepTimer: { active: false, endsAt: null, minutes: 0 } });
      get().notify(translate(get().locale, "toast.sleepCancelled"));
    },
