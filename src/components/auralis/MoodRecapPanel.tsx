...
import { useT, type Locale } from "@/lib/auralis/i18n";
...
export function MoodRecapPanel() {
  const t = useT();
  const locale = usePlayer((s) => s.locale) as Locale;
    ...
    <section className="mb-6">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--brass)]">{t("recap.eyebrow", "Bilan d’humeur")}</p>
            <h2 className="text-[20px] font-black tracking-tight text-foreground">{t("recap.title", "Ton mois en émotions")}</h2>
          </div>
        </div>
        {months.length > 0 && (
      ...
      {recap && recap.totalPlays > 0 ? (
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          ...
          >
            ...
            <div className="relative">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-white/75">
                    {recap.label}{recap.inProgress ? t("recap.inProgress", " · en cours") : ""}
                  </p>
              ...
              <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-white/85">
                <Stat value={String(recap.totalPlays)} label={t("recap.playsLabel", "écoutes")} />
                <Stat value={formatLongDuration(recap.listeningSeconds)} label={t("recap.listeningLabel", "d’écoute")} />
                <Stat value={String(recap.distinctTracks)} label={t("recap.uniqueTracksLabel", "titres uniques")} />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="matte-panel rounded-2xl p-4">
              <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/80">{t("recap.moodPalette", "Palette d’humeurs")}</p>
        ...
        <div className="matte-panel rounded-2xl p-5 text-[13px] text-muted-foreground">
          {recap?.narrative ?? t("recap.startListeningHint", "Lance quelques titres ce mois-ci pour révéler ton humeur.")}
        </div>
      )}
    </section>
  );
  ...
  return (
    <div className="matte-panel rounded-2xl p-4">
      <p className="mb-2.5 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/80">
        <Music2 className="size-3.5" /> {t("recap.monthlyTracks", "Titres du mois")}
      </p>
...
function TopArtists() {
  ...
  return (
    <div className="matte-panel rounded-2xl p-4">
      <p className="mb-2.5 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/80">
        <UserRound className="size-3.5" /> {t("recap.monthlyArtists", "Artistes du mois")}
      </p>
