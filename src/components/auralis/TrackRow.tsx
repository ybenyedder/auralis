...
import { Play, Pause, Heart, MoreHorizontal, Clock3, Check } from "lucide-react";
...
export const TrackRow = memo(function TrackRow({
  ...
  // Multi-select mode (Spotify-style) → the "AI playlist from my picks" feature.
  const selectionMode = usePlayer((s) => s.selectionMode);
  const selected = usePlayer((s) => s.selected.has(track.trackhash));
  const toggleSelected = usePlayer((s) => s.toggleSelected);
  const enterSelection = usePlayer((s) => s.enterSelection);
  ...
  // Long-press to enter selection mode on touch. A fired long-press also arms
  // suppressClick so the pointerup-triggered click doesn't immediately toggle the
  // row back off.
  ...
  const handleRowClick = () => {
    ...
    if (selectionMode) toggleSelected(track.trackhash);
    else handlePlay();
  };
    ...
    >
        ...
        {selectionMode ? (
          <span
            ...
            className={cn(
              ...
              selected ? "border-primary bg-primary text-[var(--primary-foreground)]" : "border-muted-foreground/50 text-transparent",
            )}
          >
          ...
          <button
            ...
            aria-label={isCurrentPlaying ? t("player.pause", "Pause") : t("common.playTitle", "Lire {title}", { title: trackTitle(track) })}
      ...
      {showAlbum && !compact && (
        <div className="hidden min-w-0 md:block">
          <p className="truncate text-[12px] text-muted-foreground">{track.album}</p>
        </div>
      )}
      ...
      <div className="flex items-center justify-end gap-0.5 lg:gap-1">
        {!selectionMode && (
          <button
            ...
            className={cn(
              ...
              "grid h-11 w-11 place-items-center rounded-full transition-colors duration-300 lg:h-7 lg:w-7",
              fav
                ? "text-primary opacity-100"
                : "text-muted-foreground/45 opacity-100 hover:text-foreground/90 lg:opacity-0 lg:group-hover:opacity-100",
            )}
          >
        ...
        {!selectionMode && (
          <button
            ...
            className="grid h-11 w-11 place-items-center rounded-full text-muted-foreground/55 transition-colors duration-300 hover:text-foreground/90 lg:h-7 lg:w-7 lg:text-muted-foreground/40 lg:opacity-0 lg:group-hover:opacity-100"
          >
...
export function TrackListHeader() {
  return (
    <div className="mb-1 hidden grid-cols-[24px_minmax(0,1fr)_auto] items-center gap-3 border-b border-[var(--line)] px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/70 lg:grid md:grid-cols-[24px_minmax(0,1.6fr)_minmax(0,1fr)_auto]">
