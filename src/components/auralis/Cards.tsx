...
export const AlbumCard = memo(function AlbumCard({ album, onOpen }: AlbumCardProps) {
  ...
  const isPlaying = usePlayer((s) => Boolean(s.isPlaying) && s.currentTrack?.albumhash === album.albumhash);
    ...
    >
      <div className="relative">
        <Artwork
          ...
          image={album.image}
          fluid
          className="w-full aspect-square transition-transform duration-300 ease-out group-hover:scale-[1.03]"
        />
        {/* Now-playing indicator: a subtle black overlay + animated EQ bars
            on the artwork when the album is the current one — a visual hint
            that "this is what's playing right now", painted over the cover
            instead of stealing a column from the title/artist below. */}
        {isPlaying && (
          <span
            aria-hidden
            className="absolute inset-0 grid place-items-center rounded-lg bg-black/45 backdrop-blur-[2px]"
          >
            <EqualizerBars active className="h-5" />
          </span>
        )}
        <button
          onClick={handlePlay}
          aria-label={t("common.playTitle", "Lire {title}", { title: album.title })}
          className={cn(
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 truncate text-[13px] leading-snug text-muted-foreground">
          {albumArtist(album)} · {album.year ?? t("common.unknownYear", "année inconnue")}
        </p>
      </div>
    </div>
  );
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 text-[13px] leading-snug text-muted-foreground">
          {artist.playcount ? `${t("common.playsCount", "{count} écoutes", { count: formatCount(artist.playcount) })} · ${t("common.artistLabel", "Artiste")}` : t("common.artistLabel", "Artiste")}
        </p>
      </div>
    </div>
  );
    ...
    >
      <div className="relative">
        ...
        <button
          ...
          aria-label={t("common.playTitle", "Lire {title}", { title: playlist.name })}
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 truncate text-[13px] leading-snug text-muted-foreground">
          {t("common.tracksCount", "{count} titres", { count: playlist.trackcount ?? 0 })}
        </p>
      </div>
    </div>
  );
