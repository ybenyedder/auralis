...
export function ConnectButton({ variant = "bar" }: { variant?: "bar" | "stage" }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label={t("player.devices")}
        aria-pressed={active}
        title={t("connect.choose")}
        className={cn(
          ...
          {controllingId ? (
            ...
            <>
              <div className="mb-2 flex items-center gap-2 px-1">
                ...
                <p className="text-[13px] font-bold text-white">{t("connect.title")}</p>
              </div>
              <DeviceRow current name={t("connect.thisDevice")} kind={useSync.getState().deviceKind} subtitle={t("connect.localPlayback")} onClick={() => setOpen(false)} />
              {others.length === 0 ? (
                ...
                others.map((d) => (
                  <DeviceRow
                    ...
                    name={d.name}
                    kind={d.kind}
                    subtitle={t(d.playing ? "connect.playing" : "connect.paused")}
                    playing={d.playing}
                    onClick={() => {
...
function DeviceRow({
  ...
  return (
    ...
    >
      ...
      {current && <span className="text-[10px] font-bold uppercase tracking-wide text-[var(--text-muted)]">{t("connect.here")}</span>}
    </button>
  );
}
    ...
    <div>
      <div className="mb-3 flex items-center justify-between px-1">
        <span className="flex min-w-0 items-center gap-1.5 text-[12px] font-semibold text-[var(--primary)]">
          ...
          <span className="truncate">{t("connect.listeningOn", undefined, { device: device?.name ?? t("connect.aDevice") })}</span>
        </span>
      ...
      <div className="mt-2 flex items-center justify-center gap-6">
        <button onClick={() => command("prev")} aria-label={t("player.previous")} className="text-white/80 transition-colors hover:text-white">
          <SkipBack className="size-5 fill-current" />
        </button>
        <button
          onClick={() => command(np?.isPlaying ? "pause" : "play")}
          aria-label={np?.isPlaying ? t("player.pause") : t("player.play")}
        ...
        <button onClick={() => command("next")} aria-label={t("player.next")} className="text-white/80 transition-colors hover:text-white">
      ...
      <button onClick={onClose} className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg bg-[var(--panel-3)] py-1.5 text-[11px] text-[var(--text-muted)] transition-colors hover:bg-white/10 hover:text-white">
        <X className="size-3" /> {t("player.close")}
      </button>
    </div>
  );
}
...
function RemoteLyricsPreview({
  ...
  return (
    <div className="mt-3 rounded-lg bg-[var(--panel-3)] p-2.5">
      <div className="min-h-[2.5em] px-1">
        {active ? (
          ...
          <p className="text-[12px] italic text-[var(--text-muted)]">
            {lines.length ? "…" : t("connect.noSyncLyrics")}
          </p>
        )}
      ...
      >
        <Mic2 className="size-3.5" /> {t("player.lyrics")}
      </button>
    </div>
  );
}
