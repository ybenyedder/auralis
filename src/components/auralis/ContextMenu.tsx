  ...
  return (
    <>
      ...
      <div className={cn(sheet ? "p-2" : "p-1.5", playlistOpen && "p-0")}>
        {!playlistOpen && (
          <>
            <MenuItem sheet={sheet} icon={Play} label={t("ctx.playNow", "Lire maintenant")} onClick={() => run(() => playTrack(track, [track], 0))} />
            <MenuItem sheet={sheet} icon={Radio} label={t("ctx.startRadio", "Démarrer une radio")} onClick={() => run(() => void startRadio(track.trackhash, track))} />
            <MenuItem sheet={sheet} icon={ListPlus} label={t("ctx.playNext", "Lire ensuite")} onClick={() => run(() => addToQueueNext(track))} />
            <MenuItem sheet={sheet} icon={Plus} label={t("ctx.addToQueue", "Ajouter à la file")} onClick={() => run(() => addToQueueEnd(track))} />
            <MenuItem sheet={sheet} icon={ListChecks} label={t("ctx.selectForMix", "Sélectionner pour un mix")} onClick={() => run(() => usePlayer.getState().enterSelection(track.trackhash))} />
          </>
        )}
        <PlaylistSubmenu
          submenu={submenu}
        ...
        {!playlistOpen && (
          <>
            <MenuItem
              sheet={sheet}
              icon={Heart}
              label={fav ? t("common.removeFavorite", "Retirer des favoris") : t("common.addFavorite", "Ajouter aux favoris")}
            ...
            <MenuItem
              sheet={sheet}
              icon={ThumbsDown}
              label={disliked ? t("ctx.stopDisliking", "Ne plus masquer") : t("ctx.dislike", "Je n'aime pas")}
            ...
            <MenuItem
              sheet={sheet}
              icon={Share2}
              label={t("ctx.share", "Partager")}
              onClick={() => run(() => void shareTrack(track, usePlayer.getState().notify, usePlayer.getState().locale))}
            />
            ...
            {album && (
              <MenuItem
                sheet={sheet}
                icon={Disc3}
                label={t("ctx.goToAlbum", "Aller à l'album")}
            ...
            {artist && (
              <MenuItem
                sheet={sheet}
                icon={UserRound}
                label={t("ctx.goToArtist", "Aller à l'artiste")}
  ...
  return (
    <>
      ...
      <div className={cn(sheet ? "p-2" : "p-1.5", playlistOpen && "p-0")}>
        {!playlistOpen && (
          <>
            <MenuItem sheet={sheet} icon={Play} label={t("ctx.playAlbum", "Lire l'album")} onClick={() => run(() => playList(tracks, 0))} />
            {tracks[0] && (
              <MenuItem sheet={sheet} icon={Radio} label={t("ctx.startRadio", "Démarrer une radio")} onClick={() => run(() => void startRadio(tracks[0].trackhash, tracks[0]))} />
            )}
            <MenuItem sheet={sheet} icon={ListPlus} label={t("ctx.addAlbumToQueue", "Ajouter l'album à la file")} onClick={() => run(addAlbumToQueue)} />
          </>
        )}
        <PlaylistSubmenu
          submenu={submenu}
          ...
          run={run}
          labelOverride={t("ctx.addAlbumToPlaylist", "Ajouter l'album à une playlist")}
        />
        {!playlistOpen && (
          <>
            <div className="my-1 h-px bg-[var(--line)]" />
            <MenuItem sheet={sheet} icon={Disc3} label={t("ctx.goToAlbum", "Aller à l'album")} onClick={() => run(() => navigate("album", album.albumhash))} />
            {artist && (
              <MenuItem
                sheet={sheet}
                icon={UserRound}
...
function ArtistMenu({
  ...
  return (
    <>
      <MenuHeader
        title={artist.name}
        subtitle={`${artistPlays > 0 ? `${t("common.playsCount", "{count} écoutes", { count: formatCount(artistPlays) })} · ` : ""}${artist.genres?.join(", ") || t("common.artistLabel", "Artiste")}`}
      ...
      <div className={sheet ? "p-2" : "p-1.5"}>
        <MenuItem sheet={sheet} icon={Play} label={t("ctx.playTopTracks", "Lire les titres populaires")} onClick={() => run(() => playList(topTracks, 0))} />
        {topTracks[0] && (
        ...
        {albumCount > 0 && (
          <p className={cn("text-muted-foreground/70", sheet ? "px-3 py-1.5 text-xs" : "px-2.5 py-1 text-[10.5px]")}>{t("ctx.albumsAndTracks", "{albums} albums · {tracks} titres", { albums: albumCount, tracks: artist.trackcount ?? 0 })}</p>
        )}
      </div>
    </>
  );
    ...
    if (!open) {
      return (
        <MenuItem
          sheet
          icon={ListMusic}
          label={labelOverride ?? t("ctx.addToPlaylist", "Ajouter à une playlist")}
    ...
    return (
      <div className="p-2">
        ...
        >
          ...
          {labelOverride ?? t("ctx.addToPlaylist", "Ajouter à une playlist")}
        </button>
        <div className="my-1 h-px bg-[var(--line)]" />
        <button
          role="menuitem"
        ...
        >
          <Plus className="size-5" /> {t("ctx.newPlaylist", "Nouvelle playlist")}
        </button>
        {customPlaylists.length === 0 ? (
          <p className="px-3 py-3 text-[13px] text-muted-foreground/70">{t("ctx.noPlaylists", "Aucune playlist personnelle")}</p>
        ) : (
    ...
    <div className="relative">
      ...
      {open && (
        <div className="scale-in matte-panel mb-1 ml-2 max-h-[min(360px,60vh)] overflow-y-auto scroll-auralis rounded-md">
          ...
          >
            <Plus className="size-3.5" /> {t("ctx.newPlaylist", "Nouvelle playlist")}
          </button>
          {customPlaylists.length === 0 ? (
            <p className="px-2.5 py-2 text-[11px] text-muted-foreground/70">{t("ctx.noPlaylists", "Aucune playlist personnelle")}</p>
          ) : (
            customPlaylists.map((pl) => (
              <button
                key={pl.id}
