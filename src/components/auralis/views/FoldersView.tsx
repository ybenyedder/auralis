  ...
  return (
    <div className="fade-up px-4 py-4 lg:px-6 lg:py-5">
      <div className="mb-4 lg:mb-5">
        <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--brass)]">{t("folders.eyebrow", "Fichiers")}</p>
        <h1 className="flex items-center gap-2 text-[26px] font-black tracking-tight text-foreground lg:text-[28px]">
          <Folder className="size-6 text-primary-soft" /> {t("folders.title", "Dossiers")}
        </h1>
        <p className="mt-1 text-[12px] text-muted-foreground">
          {status === "loading" ? t("folders.scanInProgress", "Scan en cours…") : root ? t("folders.sourceLabel", "Source : {root}", { root }) : t("folders.sourceMissing", "Source non configurée")}
        </p>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] lg:gap-5">
        <div className="matte-panel rounded-2xl p-2 px-1.5 lg:px-2">
          <p className="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/70">
            {t("folders.subfoldersCount", "Sous-dossiers · {count}", { count: subfolders.length })}
          </p>
          <div ref={subScrollRef} className="scroll-auralis lg:max-h-[calc(100vh-320px)] lg:overflow-y-auto">
            {subfolders.length === 0 ? (
              <div className="flex items-center justify-center gap-2 py-8 text-[12px] text-muted-foreground/70">
                <Folder className="size-4" /> {t("folders.noSubfolders", "Aucun sous-dossier")}
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 truncate text-[11px] leading-tight text-muted-foreground">
                        {child.children?.length ? t("folders.subfoldersAndTracks", "{sub} sous-dossiers · {tracks} titres", { sub: child.children.length, tracks: child.trackcount }) : t("common.tracksCount", "{count} titres", { count: child.trackcount })}
                      </p>
                    </div>
        ...
        <div className="matte-panel rounded-2xl p-2 px-1.5 lg:px-2">
          <div className="flex items-center justify-between gap-2 px-2 py-1.5">
            <p className="text-[10px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/70">
              {t("folders.tracksCount", "Titres · {count}", { count: tracksInFolder.length })}
            </p>
            <div className="flex shrink-0 items-center gap-1.5">
              ...
              >
                <Play className="size-3.5 fill-current" /> {t("folders.playFolder", "Lire le dossier")}
              </button>
              <button
                ...
                aria-label={t("folders.shuffleFolder", "Lecture aléatoire du dossier")}
              >
          ...
          <div ref={trackScrollRef} className="scroll-auralis lg:max-h-[calc(100vh-320px)] lg:overflow-y-auto">
            {tracksInFolder.length === 0 ? (
              <div className="flex items-center justify-center gap-2 py-8 text-[12px] text-muted-foreground/70">
                <Music2 className="size-4" /> {t("folders.noAudioFiles", "Aucun fichier audio")}
              </div>
            ) : (
