...
export function ArtistDetail({ artisthash }: { artisthash: string }) {
  ...
  if (!artist) return <EmptyDetail label="Artiste introuvable" loading={status !== "ready"} round />;
  ...
  const libraryRows: SettingsRow[] = [
    ...(musicDirOk === false
      ? [{
          ...
          value: "Inaccessible — la bibliothèque scannera vide",
          type: "text" as const,
...
function AccountManager() {
  ...
  const downloadBackup = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = (await res.json().catch(() => ({}))) as { error?: string };
        notify(data.error ?? "Sauvegarde impossible");
        return;
      }
  ...
  return (
    <div className="border-t border-[var(--line)] pt-5">
      ...
      <div className="mt-6 border-t border-[var(--line)] pt-5">
        <h3 className="mb-1 flex items-center gap-2 text-[14px] font-black text-foreground">
          <HardDrive className="size-4 text-primary-soft" /> Sauvegarde
        </h3>
        <p className="mb-4 text-[12px] text-muted-foreground/70">
          Télécharge une copie de la base de données (comptes, playlists, favoris, historique) —
          ne contient pas les fichiers audio eux-mêmes.
        </p>
        <button
        ...
        >
          <Download className="size-4" /> {backupBusy ? "Préparation…" : "Télécharger une sauvegarde"}
        </button>
      </div>
    </div>
  );
