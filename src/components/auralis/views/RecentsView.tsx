...
export function RecentsView() {
  ...
  const groups = useMemo(() => {
    ...
    return [
      { label: t("recents.earlier", "Plus tôt"), items: recent.slice(4, 10) },
      { label: t("recents.older", "Plus ancien"), items: recent.slice(10) },
  ...
  return (
    <div className="fade-up px-4 py-4 lg:px-6 lg:py-5">
      <div className="mb-6">
        <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--brass)]">{t("recents.eyebrow", "Historique")}</p>
        <h1 className="flex items-center gap-2 text-[28px] font-black tracking-tight text-foreground">
          <Clock3 className="size-6 text-primary-soft" /> {t("recents.title", "Récents")}
          {hasLive && (
            <span className="ml-1 rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-bold text-primary-soft">
              {t("recents.countRead", "{count} lus", { count: recent.length })}
            </span>
          )}
        </h1>
      </div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-2 md:grid-cols-4">
        {recent.slice(0, 4).map((track, index) => (
          <button
            ...
            aria-label={t("common.playTitle", "Lire {title}", { title: trackTitle(track) })}
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 flex items-center gap-1.5 truncate text-[11px] leading-tight text-muted-foreground">
                ...
                <span className="shrink-0 text-[9px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/70 lg:hidden">
                  {index === 0 ? t("recents.latest", "Dernier") : t("common.number", "#{n}", { n: index + 1 })}
                </span>
              </p>
            </div>
        ...
        {recent.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center gap-3 py-12 text-center">
            ...
            <p className="text-[13px] font-bold text-muted-foreground">{t("recents.emptyTitle", "Aucun historique réel pour l’instant")}</p>
            <p className="max-w-xs text-[12px] text-muted-foreground/70">{t("recents.emptyHint", "Lance quelques titres : ils apparaîtront ici après écoute.")}</p>
            <button
            ...
            >
              <Play className="size-4 fill-current" /> {t("recents.browseLibrary", "Parcourir la bibliothèque")}
            </button>
          </div>
        )}
      </div>
      ...
      {groups.map((group) => (
        <section key={group.label} className="mb-6">
          <SectionHeader title={group.label} eyebrow={t("recents.session", "Session")} />
