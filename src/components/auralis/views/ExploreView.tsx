  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      void api
        ...
        .catch(() => {
          if (!cancelled) setServerTracks(null);
        });
    }, 180);
    return () => {
      cancelled = true;
    ...
    >
      ...
      {best.type === "track" && (
        <button
          ...
          aria-label="Lire"
          className="signal-button glow-primary absolute bottom-5 right-5 grid size-12 translate-y-2 place-items-center rounded-full opacity-0 transition-all duration-200 group-hover:translate-y-0 group-hover:opacity-100"
        >
