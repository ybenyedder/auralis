...
export function HomeView() {
  ...
  // Cold start: the snapshot hasn't resolved yet and nothing is in memory. Show the
  // Home skeleton — NOT the "Configurer la bibliothèque" button, which used to flash
  // here (the old gate keyed on tracks.length alone) and shift the layout once the
  // library arrived. That button now belongs only to a genuinely empty, READY store.
  if ((status === "idle" || status === "loading") && tracks.length === 0) {
    return <SkeletonHome />;
  }
  ...
  return (
    <div className="relative fade-up min-h-full pb-8">
      <div className="relative z-10 px-4 py-4 lg:px-6 lg:py-6 space-y-8 lg:space-y-10">
        ...
        <section>
          {/* iOS Large Title — identical pattern to BrowseView / SearchView
              (text-[28px] font-black on mobile, lg:text-[34px]). */}
          <h1 className="mb-4 text-[28px] font-black tracking-tight text-foreground lg:text-[34px]">{greeting}</h1>
          ...
          {tracks.length === 0 ? (
            ...
            >
              ...
              {t("home.setupLibrary", "Configurer la bibliothèque")}
            </button>
          ) : quickTiles.length > 0 ? (
          ...
          <section>
            <div className="mb-1 flex items-end justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--text-muted)]">{t("home.recommendedForYou", "Recommandé pour vous")}</p>
                <h2 className="truncate text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t("home.forYou", "Fait pour vous")}</h2>
              </div>
              <button
            ...
            <div className="mb-1 flex items-end justify-between gap-3">
              <div className="flex items-center gap-3">
                <span
                  ...
                  style={{ background: moodRecs.mood.colors[0] }}
                  aria-hidden
                >
                  <Music2 className="size-6 text-white" />
                </span>
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--text-muted)]">{t("home.forMood", "Pour votre humeur")}</p>
                  <h2 className="truncate text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t(`mood.${moodRecs.mood.id}`, moodRecs.mood.label)}</h2>
                </div>
              </div>
              <button
        ...
        {featuredAlbums.length > 0 && (
          <section>
            <h2 className="text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t("home.libraryAlbums", "Albums de votre bibliothèque")}</h2>
        ...
        {featuredArtists.length > 0 && (
          <section>
            <h2 className="text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t("home.favoriteArtists", "Vos artistes préférés")}</h2>
            <div className="snap-x -mx-4 mt-4 flex gap-4 overflow-x-auto px-4 pb-1 lg:mx-0 lg:grid lg:grid-cols-6 lg:gap-4 lg:overflow-visible lg:px-0 lg:pb-0">
        ...
        {albums.length > 6 && (
          <section>
            <h2 className="text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t("home.moreAlbums", "Plus d'albums")}</h2>
            <div className="snap-x -mx-4 mt-4 flex gap-4 overflow-x-auto px-4 pb-1 lg:mx-0 lg:grid lg:grid-cols-6 lg:gap-4 lg:overflow-visible lg:px-0 lg:pb-0">
        ...
        {albums.length > 12 && (
          <section>
            <h2 className="text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t("home.recommendedAlbums", "Albums recommandés")}</h2>
            <div className="snap-x -mx-4 mt-4 flex gap-4 overflow-x-auto px-4 pb-1 lg:mx-0 lg:grid lg:grid-cols-6 lg:gap-4 lg:overflow-visible lg:px-0 lg:pb-0">
...
function QuickTile({
  ...
  return (
    <div
      ...
      className="group relative flex h-[60px] cursor-pointer items-center gap-2.5 overflow-hidden rounded-lg matte-panel-2 card-lift focus-auralis transition-all duration-300 sm:h-[64px] sm:gap-3"
    >
      {liked ? (
        <span
          className="grid h-full w-[60px] shrink-0 place-items-center bg-[var(--primary)] sm:w-[64px]"
        >
          <Heart className="size-6 fill-[var(--primary-foreground)] text-[var(--primary-foreground)]" />
        </span>
      ) : (
        <Artwork
          title={title}
          ...
          size={64}
          rounded={0}
          className="h-full w-[60px] sm:w-[64px]"
        />
      )}
      <span className="flex min-w-0 flex-1 flex-col pr-2">
        <span className="truncate text-[13px] font-semibold text-foreground sm:text-[14px]">{title}</span>
        {subtitle && <span className="truncate text-[11.5px] font-medium text-[var(--text-muted)] sm:text-[12px]">{subtitle}</span>}
      </span>
      {/* The play FAB overlays the tile's right edge on pointer devices (hidden on
          narrow touch screens — the whole tile already plays on tap; an opacity-0
          button in the flex flow would squeeze the title column). */}
      <button
        onClick={(e) => { e.stopPropagation(); onPlay(); }}
        aria-label={`Lire ${title}`}
        className="signal-button absolute right-2 top-1/2 hidden h-10 w-10 -translate-y-1/2 place-items-center rounded-full opacity-0 transition-all duration-200 group-hover:opacity-100 group-active:opacity-100 focus-visible:opacity-100 sm:grid"
      >
