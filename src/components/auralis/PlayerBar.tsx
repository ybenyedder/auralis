  ...
  return (
    <footer className="relative z-30 flex h-full w-full items-center justify-between bg-transparent px-4">
      ...
      <div className="flex min-w-0 items-center justify-start gap-4" style={{ width: "30%" }}>
        {currentTrack ? (
          <>
            <button onClick={toggleFullscreenPlayer} aria-label={t("player.enlarge", "Agrandir")} className="shrink-0 group relative overflow-hidden rounded-md focus-auralis active:scale-95 transition-all">
              <Artwork
          ...
          <p className="text-[14px] font-medium text-[var(--text-muted)]">{t("player.noPlayback", "Aucune lecture")}</p>
        )}
      </div>
      ...
      <div className="flex min-w-0 items-center justify-end gap-3" style={{ width: "30%" }}>
        ...
        <div className="relative">
          <button
            ...
            aria-label={t("player.sleep", "Minuteur de veille")}
        ...
        <div className="flex items-center gap-2 group w-[125px]">
          <button
            onClick={toggleMute}
            aria-label={muted ? t("player.unmute", "Rétablir le son") : t("player.mute", "Couper le son")}
        ...
        <button
          onClick={toggleFullscreenPlayer}
          aria-label={t("player.openFullscreen", "Plein écran")}
          className="flex items-center justify-center rounded-full p-1 text-[var(--text-muted)] transition-all active:scale-95 hover:text-white"
        >
...
function PlayerScrubber({ onSeek, disabled }: { onSeek: (seconds: number) => void; disabled: boolean }) {
  ...
  return (
    <div className="flex w-full max-w-full items-center gap-2">
      ...
      <ProgressBar
        value={value}
        valueText={t("player.playbackPositionText", "{current} sur {duration}", { current: formatDuration(shownPosition), duration: formatDuration(duration) })}
        ariaLabel={t("player.playbackPosition", "Position de lecture")}
...
function ProgressBar({
  ...
  return (
    <div
      ...
      role="slider"
      aria-label={ariaLabel ?? "Position de lecture"}
      aria-valuemin={0}
      aria-valuemax={100}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-1.5 py-1 mb-1">
        <p className="text-[11px] font-semibold text-[var(--text-muted)]">{t("player.sleep", "Minuteur de veille")}</p>
        {active && remaining && (
      ...
      {active && (
        ...
        >
          <X className="size-3" /> {t("common.cancel", "Annuler")}
        </button>
      )}
    </div>
  );
