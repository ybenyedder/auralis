...
function useVirtualWindow(
  ...
  const measure = useCallback(() => {
    ...
    // Directional overscan: extend the buffer 1.5× toward the way we're scrolling so
    // a fast fling reveals rows that are already painted instead of blank space. The
    // trailing side keeps the base overscan (cheap insurance against a bounce-back).
    const dir = viewTop - lastTop.current;
    lastTop.current = viewTop;
    const lead = Math.round(overscan * 1.5);
    const overUp = dir < 0 ? lead : overscan;
    const overDown = dir > 0 ? lead : overscan;
    let start = Math.max(0, Math.floor(rel / stride) - overUp);
    let end = Math.min(rowCount, Math.ceil((rel + viewH) / stride) + overDown);
    // When the list shrinks (tab/filter switch) below the current scrollTop, the
    // naive window can land entirely past the new end → an empty slice with no first
    // row to measure, and the list looks frozen until the browser clamps scrollTop.
    // Clamp start so we always render the tail rather than nothing.
    if (start >= rowCount) start = Math.max(0, rowCount - overscan);
    if (end < start) end = Math.min(rowCount, start + overscan);
  ...
  // A shrink clamps the scroller's scrollTop on the *next* frame, so schedule a
  // second pass in rAF to recompute the window against the corrected position —
  // otherwise the first measure runs on a stale (too large) scrollTop.
  useIsoLayoutEffect(() => {
    measure();
    const id = requestAnimationFrame(() => measure());
    return () => cancelAnimationFrame(id);
  }, [rowCount, stride, measure]);
