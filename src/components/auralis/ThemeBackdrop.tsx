...
//
// Runs on ALL platforms (web, mobile, desktop). It mounts only after hydration
// (client-only, to avoid an SSR mismatch) and returns null for matte themes, so a
// flat skin still costs nothing — the stars are back everywhere, not just Electron.
//
// Perf contract:
//   • The particle backdrops ("galaxy", "snow", "fireflies", "embers", "rain")
//     use one <canvas> + rAF — they pause when the tab is hidden and fall back to a
//     single static frame under prefers-reduced-motion. On small screens (< 768px)
//     particle counts, meteor counts and DPR are scaled down to stay light on
//     battery.
...
export function ThemeBackdrop() {
  ...
  // the client would trip a hydration mismatch. (This used to also gate on the Electron
  // bridge; the backdrop now runs on web + mobile too.)
  ...
  if (!mounted || !theme || kind === "none") return null;
...
function ParticleCanvas({
  ...
  useEffect(() => {
    ...
    const seed = () => {
      ...
      // Phones: counts already scale with area, but trim a further 40% and cap DPR
      // lower so a screen-off-then-on or a fling never spikes the battery. Meteors
      // keep at least one so a "shooting stars" theme still shoots on mobile.
      const small = w < 768;
      const budget = (small ? 0.6 : 1) * intensity;
      const meteorCount = small ? Math.max(meteors > 0 ? 1 : 0, Math.round(meteors * 0.5)) : meteors;
      ...
      if (kind === "galaxy") {
        const count = Math.min(440, Math.round((w * h) / 4200) * budget);
        ...
        meteorPool = Array.from({ length: Math.max(0, meteorCount) }, () => ({
          x: 0, y: 0, vx: 0, vy: 0, len: 0, life: 0, max: 1, color: "#fff", on: false,
        }));
      } else if (kind === "snow") {
        const count = Math.min(260, Math.round((w * h) / 7000) * budget);
        ...
        const count = Math.min(90, Math.round((w * h) / 18000) * budget);
        parts = Array.from({ length: count }, () => ({
          x: Math.random() * w,
          y: Math.random() * h,
        ...
        const count = Math.min(200, Math.round((w * h) / 9000) * budget);
        parts = Array.from({ length: count }, () => ({
          x: Math.random() * w,
          y: Math.random() * h,
        ...
        const count = Math.min(320, Math.round((w * h) / 5200) * budget);
        parts = Array.from({ length: count }, () => ({
          x: Math.random() * w,
          y: Math.random() * h,
          vx: -40,
