 ...
 */
export function MoodMixes({ title }: { title?: string }) {
  const tracks = useLibraryStore((s) => s.tracks);
  const playList = usePlayer((s) => s.playList);
  const startTrajectory = usePlayer((s) => s.startTrajectory);
  const t = useT();
  // The shelf title defaults to the translated "Selon votre humeur" unless the
  // caller passes an explicit string.
  const shelfTitle = title ?? t("home.byMood", "Selon votre humeur");
  ...
  return (
    ...
    <div className="mb-7 lg:mb-8">
      <h2 className="mb-4 text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t("home.journeys", "Voyages sonores")}</h2>
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {TRAJECTORIES.map((tr) => {
          const Icon = tr.icon;
          const label = t(`trajectory.${tr.id}`, tr.label);
          return (
            <button
              key={tr.id}
              type="button"
              onClick={() => void startTrajectory(tr.id, label)}
              aria-label={t("common.startTitle", `Démarrer : ${label}`, { title: label })}
            ...
            >
              ...
              <span className="pointer-events-none relative mt-2 block max-w-[85%] text-[16px] font-bold leading-tight text-white">{label}</span>
              <span className="pointer-events-none relative mt-0.5 block text-[12px] font-semibold text-white/80">{t(`trajectory.${tr.id}.blurb`, tr.blurb)}</span>
    ...
    <div className="mb-7 lg:mb-8">
      <h2 className="mb-4 text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{shelfTitle}</h2>
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {moods.map(({ mood, tracks: mt }) => {
          const Icon = MOOD_ICON[mood.id] ?? Music2;
          const moodLabel = t(`mood.${mood.id}`, mood.label);
          return (
          <button
            key={mood.id}
            type="button"
            onClick={() => playList(shuffleArray(mt), 0)}
            aria-label={t("common.playMix", `Lire un mix ${moodLabel}`, { mood: moodLabel })}
            className="group relative aspect-[1.1] overflow-hidden rounded-lg p-4 text-left"
            style={{ background: mood.colors[0] }}
          >
            <span className="pointer-events-none absolute inset-0 bg-black/20" aria-hidden />
            <Icon className="pointer-events-none relative size-7 text-white" />
            <span className="pointer-events-none relative mt-2 block max-w-[80%] text-[18px] font-bold leading-tight text-white">{moodLabel}</span>
            <span className="pointer-events-none relative mt-0.5 block text-[12px] font-semibold text-white/80">{t(`mood.${mood.id}.blurb`, mood.blurb)}</span>
