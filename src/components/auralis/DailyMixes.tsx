...
export function DailyMixes() {
  ...
  return (
    <section>
      <h2 className="mb-4 text-[20px] font-bold tracking-tight text-foreground lg:text-[24px]">{t("home.dailyMixes", "Vos mix du jour")}</h2>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {discoverTracks.length >= 4 && (
          <MixCard
            label={t("home.discoverWeek", "À découvrir cette semaine")}
            sublabel={t("home.unheardTracks", `{count} titres jamais écoutés`, { count: discoverTracks.length })}
        ...
        {mixes.map((m) => (
          <MixCard
            key={m.id}
            label={m.label}
            sublabel={t("home.tracksCount", `{count} titres`, { count: m.tracks.length })}
            colors={m.colors}
            covers={m.covers}
...
function MixCard({
  ...
  return (
    <button
      type="button"
      onClick={onPlay}
      aria-label={t("common.playTitle", `Lire ${label}`, { title: label })}
