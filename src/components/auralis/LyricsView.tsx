...
export function LyricsView({ variant }: { variant: "panel" | "stage" }) {
  ...
  // Server-side lyrics mode: when the instance runs with online providers
  // disabled (AURALIS_LYRICS_ONLINE=false) the "Chercher en ligne" button is a
  // dead end — hide it and hint at the .lrc sidecar instead. Optimistic true
  // until /api/health answers so the button never flickers away on normal servers.
  const [onlineLyrics, setOnlineLyrics] = useState(true);
  useEffect(() => {
    let mounted = true;
    serverInfo().then((info) => {
      if (mounted) setOnlineLyrics(info.lyricsOnline);
    });
    return () => {
      mounted = false;
    };
